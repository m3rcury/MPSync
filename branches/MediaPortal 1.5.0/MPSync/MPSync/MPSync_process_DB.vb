...
Public Class MPSync_process_DB
    ...
    Private Sub ProcessTables(ByVal source As String, ByVal target As String, ByVal database As String)
        ...
        If MPSync_process.TableExist(source, database, "mpsync_trigger") Then
            ...
            If MPSync_process.reccount.Count > 0 Then

                Dim db As Array

                For i As Integer = 0 To MPSync_process.reccount.Count - 1

                    If MPSync_process.reccount.GetByIndex(i) = -1 Then

                        db = Split(MPSync_process.reccount.GetKey(i), "|")

                        If db(0) = database Then
                            If Not parm.Contains(MPSync_process.reccount.GetKey(i)) Then
                                parm += source & "|" & target & "|" & MPSync_process.reccount.GetKey(i) & "¬"
                                MPSync_process.reccount.RemoveAt(i)
                                x += 1
                            End If
                        End If
                    End If
    ...
    Private Function Synchronize_DB(ByVal path As String, ByVal database As String, ByVal table As String, ByVal columns As Array, ByRef s_data As Array, ByRef t_data As Array, ByVal method As Integer) As Boolean
        ...
        'UpdateRecords(path, database, table, s_data, t_data, columns, method)
    ...
    Private Sub UpdateRecords(ByVal path As String, ByVal database As String, ByVal table As String, ByVal s_data As Array, ByVal t_data As Array, ByVal columns As Array, ByVal method As Integer)
        ...
        If s_data Is Nothing And t_data Is Nothing Then
            MPSync_process.logStats("MPSync: synchronization of table " & table & " in database " & path & database & " nothing to update.", "LOG")
            Exit Sub
        End If
